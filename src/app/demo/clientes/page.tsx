  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-light text-black dark:text-white">Clientes</h1>
          <p className="text-black/50 dark:text-white/50 mt-1">{clients.length} clientes registrados</p>
        </div>
        <button
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredClients.map((client) => (
          <div
            key={client.id}
            className="bg-white dark:bg-[#1a1a1a] border border-black/5 dark:border-white/5 p-6 hover:border-[#f68b44]/30 transition-colors cursor-pointer"
          ...
          >
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-medium text-black dark:text-white">{client.name}</h3>
                  <p className="text-sm text-black/50 dark:text-white/50">{client.email}</p>
                </div>
              </div>
            </div>
            ...
            <div className="mt-4 pt-4 border-t border-black/5 dark:border-white/5 flex items-center justify-between">
              <div>
                <p className="text-xs text-black/40 dark:text-white/40">Mascotas</p>
                <p className="text-sm font-medium text-black dark:text-white">{client.pets.join(", ")}</p>
              </div>
              <div className="text-right">
                <p className="text-xs text-black/40 dark:text-white/40">Total gastado</p>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-[#1a1a1a] w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-6">
              {selectedClient && (
                <div className="flex items-center gap-4 p-4 bg-[#f68b44]/5 border border-[#f68b44]/20">
                  ...
                  <div>
                    <h3 className="text-xl font-medium text-black dark:text-white">{selectedClient.name}</h3>
                    <p className="text-black/50 dark:text-white/50">Cliente desde hace 2 años</p>
                  </div>
                </div>
              )}
              ...
              <div className="grid md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium mb-2 text-black dark:text-white">DNI/NIF</label>
                  <input
                    type="text"
                    className="w-full px-4 py-3 bg-white dark:bg-[#242424] border border-black/10 dark:border-white/10 text-black dark:text-white placeholder:text-black/30 dark:placeholder:text-white/30 focus:outline-none focus:border-[#f68b44]"
              ...
              {selectedClient && (
                <div>
                  <label className="block text-sm font-medium mb-2 text-black dark:text-white">Mascotas</label>
                  <div className="flex flex-wrap gap-2">
                    ...
                    <button className="px-4 py-2 border border-dashed border-black/20 dark:border-white/20 text-black/50 dark:text-white/50 hover:border-[#f68b44] hover:text-[#f68b44] transition-colors">
                      + Añadir mascota
                    </button>
                  </div>
                </div>
              ...
              {selectedClient && (
                <div className="grid grid-cols-3 gap-4 p-4 bg-black/[0.02] dark:bg-white/[0.02]">
                  <div className="text-center">
                    <p className="text-2xl font-light text-[#f68b44]">{selectedClient.totalSpent}</p>
                    <p className="text-xs text-black/50 dark:text-white/50 mt-1">Total gastado</p>
                  </div>
                  <div className="text-center">
                    <p className="text-2xl font-light text-black dark:text-white">{selectedClient.pets.length}</p>
                    <p className="text-xs text-black/50 dark:text-white/50 mt-1">Mascotas</p>
                  </div>
                  <div className="text-center">
                    <p className="text-2xl font-light text-black dark:text-white">12</p>
                    <p className="text-xs text-black/50 dark:text-white/50 mt-1">Visitas totales</p>
                  </div>
                </div>
              )}
            </div>
